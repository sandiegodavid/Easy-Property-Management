...
| `portfolio.properties` | `properties` is a top-level portfolio aggregate with address, address-derived canonical IANA time zone, lifecycle, ownership, and reporting scope. Residential and office classifications remain fields while their core rules are shared. | The Portfolio API owns property lifecycle, local address-to-time-zone resolution, and ownership operations. Other modules consume the stored zone through application protocols rather than infer it independently. Add category-specific tables only when a future category has substantial independent fields and rules. |
