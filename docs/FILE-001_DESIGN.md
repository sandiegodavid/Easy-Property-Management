...
- `file_records` owns immutable file metadata; `file_links` connects a file to a future domain record with a purpose label. Physical deletion and retention are intentionally deferred.
...
The module does not interpret lease, payment, issue, or communication rules; later modules provide their record IDs and labels.
