...
| `base_rent_minor`, `currency_code` | Required positive amount and fixed `USD` currency code. A zero-rent agreement cannot produce FIN-001 expectations and is rejected rather than represented as recurring rent. |
...
Application commands and database constraints both enforce positive base rent and the fixed `USD` code. Booleans, floats, numeric strings, zero, and negative base-rent inputs are invalid. Security deposits and optional termination fees remain non-negative because zero has a distinct valid meaning for those fields.
...
6. **Harden FIN-001 source terms.** Require positive integer base rent and the fixed `USD` code in the HTTP model, direct application command, SQLAlchemy constraints, greenfield baseline, and exact-schema validator. Regression tests must reject booleans, floats, numeric strings, zero/negative rent, and every currency value other than `USD`.
