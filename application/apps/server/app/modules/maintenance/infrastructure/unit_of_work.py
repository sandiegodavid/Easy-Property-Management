...
class SQLiteMaintenanceTransaction:
    def __init__(self, connection, recorder, portfolio, finance, tasks, task_operations, files):
    ...
    def issues_page(self, *, property_id=None, space_id=None, category=None, priority=None, status=None, reported_from=None, reported_to=None, appointment_from=None, appointment_to=None, has_evidence=None, has_linked_expense=None, has_active_task=None, cursor=None, limit=101):
        query=select(MaintenanceIssueModel)
        conditions=[]
        for column,value in ((MaintenanceIssueModel.property_id,property_id),(MaintenanceIssueModel.space_id,space_id),(MaintenanceIssueModel.category,category),(MaintenanceIssueModel.priority,priority),(MaintenanceIssueModel.status,status)):
    ...
    def projection(self, issue_ids):
        ...
        return {issue_id:{"issue":issue,"appointments":appointments[issue_id],"costs":costs[issue_id],"links":links[issue_id],"context":contexts.get((issue["property_id"],issue["space_id"])),"tasks":tasks.get(issue_id,[]),"expenses":expenses,"files":issue_files.get(issue_id,[]),"appointment_files":appointment_files,"cost_files":cost_files} for issue_id,issue in issues.items()}
...
class SQLiteMaintenanceUnitOfWork:
    def __init__(self,database,recorder:AuditRecorder,portfolio,finance,tasks,task_operations,files=None):
        self.database,self.recorder=database,recorder
        self.portfolio,self.finance,self.tasks,self.task_operations,self.files=portfolio,finance,tasks,task_operations,files
    def write(self, operation):
        engine=create_engine(f"sqlite:///{self.database}")
        try:
            with immediate_transaction(engine) as connection:return operation(SQLiteMaintenanceTransaction(connection,self.recorder,self.portfolio,self.finance,self.tasks,self.task_operations,self.files))
        finally:engine.dispose()
    def read(self, operation):
        engine=create_engine(f"sqlite:///{self.database}")
        try:
            with engine.connect() as connection:return operation(SQLiteMaintenanceTransaction(connection,self.recorder,self.portfolio,self.finance,self.tasks,self.task_operations,self.files))
        finally:engine.dispose()
