...
def build_router(service:AiGovernanceService,runtime:WorkspaceRuntime)->APIRouter:
    ...
    def invoke(call):
        ...
        except AiValidationError as error:raise HTTPException(400,{"code":error.code,"message":str(error)}) from error
    ...
    def settings():ready();return invoke(service.settings)
    ...
    def patch_connection(connection_id:UUID,data:ConnectionPatch):
        ready(True);payload=data.model_dump(exclude={"expectedRevision"},exclude_unset=True);names={"modelArtifactDigest":"model_artifact_digest","runtimeId":"runtime_id","runtimeVersion":"runtime_version"};return invoke(lambda:service.update_connection(str(connection_id),expected_revision=data.expectedRevision,data={names.get(key,key):value for key,value in payload.items()}))
    @router.put("/connections/{connection_id}/disclosure",operation_id="recordAiDisclosure")
    def disclosure(connection_id:UUID,data:DisclosureInput):ready(True);return invoke(lambda:service.set_disclosure(str(connection_id),expected_revision=data.expectedRevision,disclosure_version=data.disclosureVersion,data_classes=data.dataClasses))
    @router.put("/connections/{connection_id}/credential",status_code=204,operation_id="setAiCredential")
    def credential(connection_id:UUID,data:CredentialInput):ready(True);invoke(lambda:service.set_credential(str(connection_id),data.credential))
    @router.delete("/connections/{connection_id}/credential",status_code=204,operation_id="deleteAiCredential")
    def delete_credential(connection_id:UUID):ready(True);invoke(lambda:service.delete_credential(str(connection_id)))
    @router.post("/connections/{connection_id}/test",operation_id="testAiConnection")
    def test_connection(connection_id:UUID):ready();return invoke(lambda:service.test_connection(str(connection_id)))
    @router.get("/limits",operation_id="listAiActionLimits")
    def limits():ready();return invoke(service.limits)
    @router.put("/limits/{action_type}",operation_id="updateAiActionLimit")
    def limit(action_type:str,data:LimitInput):ready(True);return invoke(lambda:service.put_limit(action_type,{"enabled":data.enabled,"connection_id":None if data.connectionId is None else str(data.connectionId),"max_runs_per_utc_day":data.maxRunsPerUtcDay,"max_prompt_tokens":data.maxPromptTokens,"max_completion_tokens":data.maxCompletionTokens,"allowed_models":data.allowedModels}))
    @router.get("/redaction-profiles",operation_id="listAiRedactionProfiles")
    def profiles():ready();return [{"name":item.name,"version":item.version,"fieldHandling":dict(item.summary)} for item in service.profiles.all()]
    ...
    def drafts(status:str|None=None,owningModule:str|None=None,entityKind:str|None=None,actionType:str|None=None,sourceEntityType:str|None=None,sourceEntityId:str|None=None,cursor:str|None=None,pageSize:int=Query(100,ge=1,le=500)):
        ...
        return invoke(lambda:service.list_drafts(status=status,owning_module=owningModule,entity_kind=entityKind,action_type=actionType,source_entity_type=sourceEntityType,source_entity_id=sourceEntityId,cursor=parsed,page_size=pageSize))
    @router.get("/drafts/{draft_id}",operation_id="getAiDraft")
    def draft(draft_id:UUID):ready();return invoke(lambda:service.draft_detail(str(draft_id)))
    @router.patch("/drafts/{draft_id}",operation_id="editAiDraft")
    def edit(draft_id:UUID,data:DraftEditInput):ready(True);return invoke(lambda:service.edit_draft(str(draft_id),version=data.version,payload=data.draftPayload,operator_note=data.operatorNote))
    @router.post("/drafts/{draft_id}/approve",operation_id="approveAiDraft")
    def approve(draft_id:UUID,data:DraftDecisionInput):ready(True);return invoke(lambda:service.approve_draft(str(draft_id),version=data.version,operator_note=data.operatorNote))
    @router.post("/drafts/{draft_id}/dismiss",operation_id="dismissAiDraft")
    def dismiss(draft_id:UUID,data:DraftDecisionInput):ready(True);return invoke(lambda:service.dismiss_draft(str(draft_id),version=data.version,operator_note=data.operatorNote))
    return router
