...
class TaskActivitySnapshotPolicy(DefaultAuditSnapshotPolicy):
    """Keep ordinary tasks visible while hiding communication follow-up linkage."""

    @staticmethod
    def _communication_follow_up(snapshot: Mapping[str, Any] | None) -> bool:
        return bool(snapshot and snapshot.get("relatedEntityType") == "communication")

    def redact(self, snapshot: Mapping[str, Any] | None) -> dict[str, Any] | None:
        value = super().redact(snapshot)
        if value is None or not self._communication_follow_up(snapshot):
            return value
    ...
    def redact_entity_id(
        ...
        return "[redacted]" if self._communication_follow_up(after or before) else entity_id
