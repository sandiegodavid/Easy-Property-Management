...
class OwnerConcernModel(LocalBase):
    ...
    __table_args__ = (
        ...
        CheckConstraint("(space_id IS NULL AND space_display_name_snapshot IS NULL) OR (space_id IS NOT NULL AND length(trim(space_display_name_snapshot)) BETWEEN 1 AND 240)"),
        CheckConstraint("(lease_id IS NULL AND lease_display_snapshot IS NULL) OR (lease_id IS NOT NULL AND length(trim(lease_display_snapshot)) BETWEEN 1 AND 240)"),
        CheckConstraint("(tenant_party_id IS NULL AND tenant_display_name_snapshot IS NULL) OR (tenant_party_id IS NOT NULL AND length(trim(tenant_display_name_snapshot)) BETWEEN 1 AND 240)"),
        ...
        CheckConstraint("(status='resolved' AND resolved_at_utc IS NOT NULL AND length(trim(resolution_summary)) BETWEEN 1 AND 4000 AND dismissed_at_utc IS NULL AND dismissal_reason IS NULL) OR (status='dismissed' AND dismissed_at_utc IS NOT NULL AND length(trim(dismissal_reason)) BETWEEN 1 AND 4000 AND resolved_at_utc IS NULL AND resolution_summary IS NULL) OR (status IN ('open','in_progress') AND resolved_at_utc IS NULL AND resolution_summary IS NULL AND dismissed_at_utc IS NULL AND dismissal_reason IS NULL)"),
        ...
        CheckConstraint("(concern_type='vacancy' AND observed_occupancy_status IS NOT NULL AND observed_availability_status IS NOT NULL) OR (concern_type!='vacancy' AND observed_occupancy_status IS NULL AND observed_availability_status IS NULL AND observed_available_on IS NULL)"),
