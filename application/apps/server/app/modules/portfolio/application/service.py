...
from app.modules.portfolio.application.status_read_model import space_status_snapshot, space_status_view
...
@dataclass(frozen=True)
class OccupancyCorrectionCommand:
    occupancy: OccupancyCommand
    reason: str

    def __post_init__(self) -> None:
        if not isinstance(self.occupancy, OccupancyCommand):
            raise PortfolioError("Occupancy correction requires a valid occupancy value.")
        object.__setattr__(self, "reason", _required(self.reason, "Correction reason", 1000))
...
class PortfolioService:
    def __init__(self, unit_of_work: PortfolioUnitOfWork, party_factory: PartyFactory | None = None,
                 ...
                 time_zone_resolver: AddressTimeZoneResolver | None = None,
    ...
    def create_property(self, command: PropertyCreateCommand) -> Property:
        ...
        def write(transaction: PortfolioTransaction) -> Property:
            ...
            for ownership in self._new_ownerships(property.id, ownership_inputs, effective_on, now):
            ...
            for space, space_command in zip(new_spaces, command.spaces or (), strict=True):
                ...
                self._insert_initial_statuses(transaction, space, space_command, now, correlation_id, effective_on)
            return property
        return self.unit_of_work.write(write)
    ...
    def get_property(self, property_id: str) -> dict[str, object]:
        ...
        return self._with_statuses(self._property_view(property, ownerships, parties, spaces, as_of=as_of), spaces, as_of=as_of)
    ...
    def list_properties(
        ...
        *,
        ...
        if availability_filter is not None and availability_filter not in {"available_now", "available_on", "not_available", "unknown"}:
            raise PortfolioError("Availability status is invalid.")
        as_of = as_of or self._instant()
        def read(transaction: PortfolioTransaction) -> list[dict[str, object]]:
            raw = transaction.property_views(status=status)
            all_spaces = [space for _, _, _, spaces in raw for space in spaces]
            period_map, availability_map = transaction.space_statuses([space.id for space in all_spaces])
            records = [self._with_statuses(self._property_view(property, ownerships, parties, spaces, as_of=as_of), spaces, period_map, availability_map, as_of=as_of) for property, ownerships, parties, spaces in raw]
            filtered = []
            for record in records:
                ...
                if ownership_context_filter is not None and record["ownershipContext"] != ownership_context_filter:
                    continue
                if occupancy_filter is not None and not any(space["currentOccupancy"]["occupancyStatus"] == occupancy_filter for space in active_spaces):
                    continue
                if availability_filter is not None and not any(space["availability"]["availabilityStatus"] == availability_filter for space in active_spaces):
                    continue
    ...
    def page_properties(self, *, status: str | None = None, ownership_context_filter: str | None = None,
                        occupancy_filter: str | None = None, availability_filter: str | None = None,
                        needs_attention: bool | None = None, page_size: int = 100,
                        cursor: str | None = None) -> dict[str, object]:
        if type(page_size) is not int or not 1 <= page_size <= 500:
            raise PortfolioError("Page size must be between 1 and 500.")
        if status is not None and status not in {"active", "archived"}: raise PortfolioError("Property status is invalid.")
        if ownership_context_filter is not None and ownership_context_filter not in {"self_owned", "managed_for_owner", "mixed"}: raise PortfolioError("Ownership context is invalid.")
        if occupancy_filter is not None and occupancy_filter not in {"occupied", "vacant", "unknown"}: raise PortfolioError("Occupancy status is invalid.")
        if availability_filter is not None and availability_filter not in {"available_now", "available_on", "not_available", "unknown"}: raise PortfolioError("Availability status is invalid.")
        as_of = self._instant()
        marker = None if cursor is None else _property_cursor(cursor)
        def read(transaction: PortfolioTransaction) -> dict[str, object]:
            local_days = {zone: as_of.astimezone(ZoneInfo(zone)).date().isoformat() for zone in transaction.property_time_zones()}
            if not local_days:
                return {"items": [], "nextCursor": None, "matchingTotal": 0, "asOf": as_of.isoformat()}
            ids, total = transaction.property_page_ids(status=status, ownership_context=ownership_context_filter, occupancy=occupancy_filter, availability=availability_filter, needs_attention=needs_attention, local_days=local_days, cursor=marker, limit=page_size + 1)
            has_next = len(ids) > page_size
            raw = transaction.property_views_for_ids(ids[:page_size])
            all_spaces = [space for _, _, _, spaces in raw for space in spaces]
            period_map, availability_map = transaction.space_statuses([space.id for space in all_spaces])
            items = [self._with_statuses(self._property_view(property, ownerships, parties, spaces, as_of=as_of), spaces, period_map, availability_map, as_of=as_of) for property, ownerships, parties, spaces in raw]
            next_cursor = None if not has_next or not items else _encode_property_cursor(items[-1])
            return {"items": items, "nextCursor": next_cursor, "matchingTotal": total, "asOf": as_of.isoformat()}
        return self.unit_of_work.read(read)
    ...
    def add_space(self, property_id: str, command: SpaceCreateCommand) -> Space:
        ...
        def write(transaction: PortfolioTransaction) -> Space:
            ...
            transaction.record_change(entity_type="space", entity_id=space.id, action="created",
                                      ...
                                      correlation_id=correlation_id)
            self._insert_initial_statuses(
                transaction, space, command, now, correlation_id,
                as_of.astimezone(ZoneInfo(property.time_zone)).date().isoformat(),
            )
            return space
    ...
    def archive_space(self, space_id: str, *, confirmed: bool) -> Space:
        ...
        def write(transaction: PortfolioTransaction) -> Space:
            ...
            self._assert_space_can_archive(transaction, current, self._local_day(transaction, current, as_of))
            archived = replace(
                current,
                status="archived",
                updated_at=now,
    ...
    def _instant(self) -> datetime:
        ...
        if not isinstance(instant, datetime) or instant.tzinfo is None or instant.utcoffset() is None:
            raise PortfolioError("Portfolio clock must return an aware timestamp.")
        return instant.astimezone(UTC)

    @staticmethod
    def _local_day(transaction: PortfolioTransaction, space: Space, as_of: datetime) -> str:
        property = transaction.get_property(space.property_id)
        if property is None:
            raise PortfolioError("Space property is missing.")
        return as_of.astimezone(ZoneInfo(property.time_zone)).date().isoformat()

    @staticmethod
    def _supersede_period(
        transaction: PortfolioTransaction,
        superseded: SpaceOccupancyPeriod,
        replacement: SpaceOccupancyPeriod,
    ) -> None:
        # Stage the replacement outside the valid timeline so the self-reference
        # and single-open-period constraints are true at every write.
        transaction.insert_occupancy_period(replace(replacement, record_state="superseded"))
        transaction.replace_occupancy_period(superseded)
        transaction.replace_occupancy_period(replacement)

    def _prepare_status_mutation(
        self, transaction: PortfolioTransaction, space: Space, expected_revision: int,
        idempotency_key: str, payload: dict[str, object], as_of: datetime,
        ...
        if existing is not None:
            if existing["space_id"] != space.id:
                raise PortfolioConflictError("This idempotency key was already used for another space.")
            if existing["request_fingerprint"] != fingerprint:
                raise PortfolioConflictError("This idempotency key was already used with a different request.")
            return fingerprint, loads(str(existing["result_snapshot"]))
        if space.status_revision != expected_revision:
            effective_on = self._local_day(transaction, space, as_of)
            snapshot = self._temporal_status(
                self._space_status_view(
                    space, transaction.occupancy_periods(space.id), transaction.availability(space.id),
    ...
    def _status_mutation_finish(
        transaction: PortfolioTransaction, space: Space, now: str, idempotency_key: str,
        fingerprint: str, result: dict[str, object],
    ) -> dict[str, object]:
        revised = replace(space, status_revision=space.status_revision + 1, updated_at=now)
        transaction.replace_space(revised)
        result = {
            **result,
        ...
        transaction.insert_status_operation({
            "id": result["operationId"], "space_id": space.id, "idempotency_key": idempotency_key,
            "request_fingerprint": fingerprint, "result_revision": revised.status_revision,
            "result_snapshot": dumps(result, sort_keys=True, separators=(",", ":")), "created_at": now,
        })
        return result

    @staticmethod
    def _temporal_status(result: dict[str, object], as_of: datetime, effective_on: str) -> dict[str, object]:
    ...
    def _change_property_status(self, property_id: str, status: str) -> Property:
        ...
        def write(transaction: PortfolioTransaction) -> Property:
            current = transaction.get_property(property_id)
            if current is None: raise KeyError
            today = as_of.astimezone(ZoneInfo(current.time_zone)).date().isoformat()
            if current.status == status: return current
            if status == "archived":
                conflicts = transaction.property_archive_conflicts(property_id)
                if conflicts:
                    raise PortfolioError(conflicts[0])
            if status == "active" and not transaction.ownerships_at(property_id, today):
            ...
            if status == "archived":
                for space in transaction.spaces(property_id, active_only=True):
                    self._assert_space_can_archive(transaction, space, today)
    ...
    def replace_ownerships(self, property_id: str, ownerships: tuple[OwnershipInput, ...], effective_on: str) -> dict[str, object]:
        _validate_ownerships(ownerships); effective = _date(effective_on); correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        def write(transaction: PortfolioTransaction) -> list[PropertyOwnership]:
            property = transaction.get_property(property_id)
            if property is None: raise KeyError
            today = as_of.astimezone(ZoneInfo(property.time_zone)).date().isoformat()
    ...
    def get_space_status(self, space_id: str) -> dict[str, object]:
        ...
        property = self.unit_of_work.get_property(space.property_id)
        if property is None:
            raise PortfolioNotFoundError("Property was not found.")
        as_of = self._instant()
        return space_status_snapshot(space, property, periods, availability, as_of)

    def portfolio_status_summary(self) -> dict[str, object]:
        as_of = self._instant()
        properties = self.list_properties(status="active", as_of=as_of)
    ...
    def change_occupancy(self, space_id: str, command: OccupancyCommand, *, expected_revision: int, idempotency_key: str) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        def write(transaction: PortfolioTransaction):
            space = transaction.get_space(space_id)
            if space is None: raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "occupancy_changed", "command": repr(command)}, as_of)
            if replay is not None: return replay
            today = self._local_day(transaction, space, as_of)
            if command.effective_on < today:
            ...
            result = self._temporal_status(self._space_status_view(space, transaction.occupancy_periods(space_id), transaction.availability(space_id), effective_on=today), as_of, today)
            return self._status_mutation_finish(transaction, space, now, idempotency_key, fingerprint, result)
        return self._not_found_from_key_error(write, label="Space")

    def cancel_scheduled_occupancy(self, space_id: str, period_id: str, *, expected_revision: int, idempotency_key: str) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        def write(transaction: PortfolioTransaction):
            space = transaction.get_space(space_id)
            if space is None: raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "occupancy_cancelled", "periodId": period_id}, as_of)
            if replay is not None: return replay
            today = self._local_day(transaction, space, as_of)
            periods = transaction.occupancy_periods(space_id)
            ...
            if period.record_state != "valid" or period.starts_on <= today:
                raise PortfolioError("Only a future manual occupancy transition can be cancelled.")
            future = [item for item in periods if item.record_state == "valid" and item.starts_on > today]
    ...
    def replace_scheduled_occupancy(
        ...
        *, expected_revision: int, idempotency_key: str,
        ...
        def write(transaction: PortfolioTransaction):
            ...
            if space is None:
                raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "occupancy_replaced", "periodId": period_id, "command": repr(command)}, as_of)
            if replay is not None: return replay
            today = self._local_day(transaction, space, as_of)
            periods = transaction.occupancy_periods(space_id)
            period = next((item for item in periods if item.id == period_id), None)
            ...
            self._supersede_period(transaction, superseded, replacement)
            transaction.record_change(
            ...
            result = self._temporal_status(self._space_status_view(
                space,
                transaction.occupancy_periods(space_id),
                transaction.availability(space_id), effective_on=today,
            ), as_of, today)
            return self._status_mutation_finish(transaction, space, now, idempotency_key, fingerprint, result)
    ...
    def correct_occupancy(
        self, space_id: str, period_id: str, command: OccupancyCorrectionCommand, *, expected_revision: int, idempotency_key: str,
    ) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        ...
        def write(transaction: PortfolioTransaction):
            ...
            if space is None:
                raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "occupancy_corrected", "periodId": period_id, "command": repr(command)}, as_of)
            if replay is not None: return replay
            periods = transaction.occupancy_periods(space_id)
            period = next((item for item in periods if item.id == period_id), None)
            if period is None:
            ...
            today = self._local_day(transaction, space, as_of)
            if (
                period.source_kind != "manual"
                or period.record_state != "valid"
                or not (period.starts_on <= today and (period.ends_on is None or period.ends_on > today))
            ):
                raise PortfolioConflictError("Only a current manual occupancy transition can be corrected.")
            if command.occupancy.effective_on != period.starts_on:
                raise PortfolioError("A correction must keep the original effective date.")
            replacement = SpaceOccupancyPeriod(
                id=str(uuid4()), space_id=space_id,
                occupancy_status=command.occupancy.occupancy_status, starts_on=period.starts_on,
                ends_on=period.ends_on, record_state="valid", superseded_by_id=None,
                source_kind="manual", source_id=None, note=command.occupancy.note,
                created_at=now, ended_at=period.ended_at, cancelled_at=None,
            )
            superseded = replace(period, record_state="superseded", superseded_by_id=replacement.id)
            self._supersede_period(transaction, superseded, replacement)
            transaction.record_change(entity_type="space_occupancy", entity_id=period.id,
                                      action="superseded", before=period.to_dict(), after=superseded.to_dict(),
                                      reason=command.reason, correlation_id=correlation_id)
            transaction.record_change(entity_type="space_occupancy", entity_id=replacement.id,
                                      action="created", before=None, after=replacement.to_dict(),
                                      reason=command.reason, correlation_id=correlation_id)
            result = self._temporal_status(self._space_status_view(space, transaction.occupancy_periods(space_id), transaction.availability(space_id), effective_on=today), as_of, today)
            return self._status_mutation_finish(transaction, space, now, idempotency_key, fingerprint, result)
    ...
    def reschedule_scheduled_occupancy(
        self, space_id: str, period_id: str, command: OccupancyCommand, *, expected_revision: int, idempotency_key: str,
    ) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        ...
        def write(transaction: PortfolioTransaction):
            ...
            if space is None:
                raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "occupancy_rescheduled", "periodId": period_id, "command": repr(command)}, as_of)
            if replay is not None: return replay
            periods = transaction.occupancy_periods(space_id)
            period = next((item for item in periods if item.id == period_id), None)
            if period is None:
            ...
            if period.source_kind != "manual":
                raise PortfolioConflictError("Source-owned occupancy transitions must be changed by their owning module.")
            if period.record_state != "valid" or not future or period.id != max(future, key=lambda item: item.starts_on).id:
                raise PortfolioError("Only the latest future manual transition can be rescheduled.")
            if command.effective_on <= today:
                raise PortfolioError("A scheduled transition must remain in the future.")
            previous = next((item for item in periods if item.record_state == "valid" and item.ends_on == period.starts_on), None)
            if previous is None:
                raise PortfolioError("Scheduled occupancy has no preceding period.")
            if command.effective_on <= previous.starts_on:
                raise PortfolioError("A rescheduled transition cannot precede its prior occupancy period.")
            if period.ends_on is not None and command.effective_on >= period.ends_on:
                raise PortfolioError("A rescheduled transition must remain before its following transition.")
            replacement = SpaceOccupancyPeriod(
                id=str(uuid4()), space_id=space_id, occupancy_status=command.occupancy_status,
                starts_on=command.effective_on, ends_on=period.ends_on, record_state="valid",
                superseded_by_id=None, source_kind="manual", source_id=None, note=command.note,
                created_at=now, ended_at=None, cancelled_at=None,
            )
            superseded = replace(period, record_state="superseded", superseded_by_id=replacement.id)
            adjusted_previous = replace(previous, ends_on=command.effective_on, ended_at=now)
            transaction.replace_occupancy_period(adjusted_previous)
            self._supersede_period(transaction, superseded, replacement)
            transaction.record_change(entity_type="space_occupancy", entity_id=previous.id,
                                      action="ended", before=previous.to_dict(), after=adjusted_previous.to_dict(),
                                      reason="occupancy_scheduled_rescheduled", correlation_id=correlation_id)
            transaction.record_change(entity_type="space_occupancy", entity_id=period.id,
                                      action="superseded", before=period.to_dict(), after=superseded.to_dict(),
                                      reason="occupancy_scheduled_rescheduled", correlation_id=correlation_id)
            transaction.record_change(entity_type="space_occupancy", entity_id=replacement.id,
    ...
    def change_availability(self, space_id: str, command: AvailabilityCommand, *, expected_revision: int, idempotency_key: str) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        def write(transaction: PortfolioTransaction):
            space = transaction.get_space(space_id)
            if space is None: raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "availability_changed", "command": repr(command)}, as_of)
            if replay is not None: return replay
            today = self._local_day(transaction, space, as_of)
            if command.available_on is not None and command.available_on < today:
            ...
            result = self._temporal_status(self._space_status_view(space, transaction.occupancy_periods(space_id), updated, effective_on=today), as_of, today)
            return self._status_mutation_finish(transaction, space, now, idempotency_key, fingerprint, result)
        return self._not_found_from_key_error(write, label="Space")

    def classify_space(self, space_id: str, command: SpaceClassificationCommand, *, expected_revision: int, idempotency_key: str) -> dict[str, object]:
        correlation_id, as_of = str(uuid4()), self._instant()
        now = as_of.isoformat()
        ...
        def write(transaction: PortfolioTransaction):
            ...
            if space is None:
                raise KeyError
            fingerprint, replay = self._prepare_status_mutation(transaction, space, expected_revision, idempotency_key, {"action": "space_classified", "command": repr(command)}, as_of)
            if replay is not None: return replay
            today = self._local_day(transaction, space, as_of)
            if command.occupancy is not None and command.occupancy.effective_on != today:
            ...
            if command.occupancy is not None:
                ...
                if current.starts_on == today:
                    ...
                    ended = replace(current, ends_on=today, ended_at=now)
            ...
            result = self._temporal_status(self._space_status_view(space, transaction.occupancy_periods(space_id), updated_availability, effective_on=today), as_of, today)
            return self._status_mutation_finish(transaction, space, now, idempotency_key, fingerprint, result)
    ...
    def _insert_initial_statuses(
        ...
        occupancy_command = command.initial_occupancy or OccupancyCommand("unknown", effective_on)
        if occupancy_command.effective_on > effective_on:
            raise PortfolioError("Initial occupancy cannot begin in the future.")
        if availability_command := command.initial_availability:
            if availability_command.available_on is not None and availability_command.available_on < effective_on:
                raise PortfolioError("An availability date cannot be in the past.")
    ...
    @staticmethod
    def _assert_space_can_archive(transaction: PortfolioTransaction, space: Space, today: str) -> None:
    ...
    @staticmethod
    def _space_status_view(space: Space, periods: list[SpaceOccupancyPeriod], availability: SpaceAvailability | None,
                           *, effective_on: str) -> dict[str, object]:
        try:
            return space_status_view(space, periods, availability, effective_on=effective_on)
    ...
    def _with_statuses(
        ...
        *, as_of: datetime | None = None,
        ...
        as_of = as_of or self._instant()
        effective_on = as_of.astimezone(ZoneInfo(str(record["timeZone"]))).date().isoformat()
        status_spaces = [self._temporal_status(self._space_status_view(
            space, period_map.get(space.id, []), availability_map.get(space.id), effective_on=effective_on,
        ), as_of, effective_on) for space in spaces]
        ...
        return {**record, "spaces": status_spaces, "statusSummary": summary,
                "asOf": as_of.isoformat(), "effectiveLocalDate": effective_on}
    ...
    def _property_view(
        self, property: Property, ownerships: list[PropertyOwnership], parties: dict[str, Party],
        spaces: list[Space] | None = None, *, as_of: datetime | None = None,
    ) -> dict[str, object]:
        instant = as_of or self._instant()
        current = [item for item in ownerships if active_on(item, instant.astimezone(ZoneInfo(property.time_zone)).date())]
...
def _encode_property_cursor(record: dict[str, object]) -> str:
    return urlsafe_b64encode(dumps((str(record["displayName"]), str(record["id"]))).encode()).decode()


def _property_cursor(value: str) -> tuple[str, str]:
    try:
        name, property_id = loads(urlsafe_b64decode(value.encode()).decode())
    except (TypeError, ValueError, UnicodeDecodeError) as error:
        raise PortfolioError("Property cursor is invalid.") from error
    if not isinstance(name, str) or not isinstance(property_id, str) or not name or not property_id:
        raise PortfolioError("Property cursor is invalid.")
    return name, property_id
